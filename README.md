...
|---|---|---|---|
| `0a3304f` | 2026-06-19 | Broken jump after level re-entry | Re-entering a level as a living player normalizes the player's size like a fresh spawn — but it forgot `.scale`, so a corrupt negative scale carried over from the previous level survived the transition. Since the jump impulse is `270 * self.scale`, a negative scale launched the player *downward* instead of up, leaving jumping broken until the next clean spawn. `ClientReEnter` now clamps a non-positive scale back to 1 (as `PutClientInServer` already does). Symptom fix — the upstream corruptor that writes the bad scale is still unidentified. |
| `b9e5573` | 2026-06-15 | Vorpal double mana charge | Un-tomed, a connecting Vorpal swing is meant to cost 2 blue mana — but vanilla billed it off a stale `trace_ent` left over after the splash loop, so a hit cost an erratic 2–4 depending on what the trace last touched, and the tail drain double-charged the direct hit that `vorpal_melee` had already billed. Now deterministic: a direct hit costs 2, +2 only when the swing also splashes *other* nearby monsters, a clean miss 0. Tome path untouched. |
